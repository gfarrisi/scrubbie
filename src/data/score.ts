...
export type Metric = 'walletAge' | 'socialProfile' | 'numPurchases' | 'pricePurchases' | 'timeZoneCluster'
...
export const weightAndThresholdDefaults = (scrubScoreProps: ScrubScoreProps, 
    metric: Metric
    ): WeightAndThreshold => {
    const weight = scrubScoreProps.weights[metric]
    const threshold = metric === 'socialProfile' || metric === 'timeZoneCluster' ? null : scrubScoreProps.threshold[metric]
    return {
        weight,
        threshold
    }
...
export const getScrubScore = async (scrubScoreProps: ScrubScoreProps) => {
    ...
    const socialScore = await socialProfileScore(walletData, weightAndThresholdDefaults(scrubScoreProps, 'socialProfile'))
